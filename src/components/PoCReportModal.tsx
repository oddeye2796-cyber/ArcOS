      ...
      <div className="bg-white rounded-2xl shadow-2xl border border-slate-200 w-full max-w-3xl overflow-hidden flex flex-col max-h-[92vh]">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] px-1.5 py-0.2 rounded bg-emerald-500/20 text-emerald-300 border border-emerald-400/30">
                  {t.pocReportStatus.replace('{elapsed}', String(daysElapsed)).replace('{remaining}', String(trial.daysRemaining))}
                </span>
              </div>
        ...
        <div className="p-6 overflow-y-auto space-y-6 text-xs text-slate-700 flex-1">
          ...
          <div className="p-4 bg-slate-50 border border-slate-200 rounded-xl space-y-3">
            <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-200 pb-2.5">
              <div>
                <span className="text-[11px] text-slate-500 block">{t.pocReportFacilityNode}</span>
              ...
              <div>
                ...
                <span className="font-mono text-slate-800">
                  {trial.startedAt} ~ {trial.expiresAt} ({t.pocReportPeriodUnit})
                </span>
              </div>
            </div>
          ...
          <div>
            <h3 className="font-bold text-slate-900 text-xs mb-3 flex items-center gap-1.5">
              ...
              <span>{t.pocReportMetricsTitle}</span>
            </h3>

            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              <div className="p-3.5 bg-emerald-50/70 border border-emerald-200 rounded-xl">
                <span className="text-[11px] text-emerald-800 font-medium block">{t.pocMetricTechFitScore}</span>
                <div className="text-2xl font-bold font-mono text-emerald-700 mt-1">
                  {trial.healthScore}%
                </div>
                <span className="text-[10px] text-emerald-600 font-medium mt-0.5 block">
                  {t.pocMetricTechFitDesc}
                </span>
              </div>

              <div className="p-3.5 bg-blue-50/70 border border-blue-200 rounded-xl">
                <span className="text-[11px] text-blue-800 font-medium block">{t.pocMetricOntologyTags}</span>
                ...
                <span className="text-[10px] text-blue-600 font-medium mt-0.5 block">
                  {t.pocMetricOntologyTagsDesc}
                </span>
              </div>

              <div className="p-3.5 bg-indigo-50/70 border border-indigo-200 rounded-xl">
                <span className="text-[11px] text-indigo-800 font-medium block">{t.pocMetricDowntime}</span>
          ...
          <div className="space-y-2">
            <h3 className="font-bold text-slate-900 text-xs flex items-center gap-1.5">
              ...
              <span>{t.pocReportCriteriaTitle}</span>
            </h3>
            ...
            <div className="border border-slate-200 rounded-xl overflow-hidden divide-y divide-slate-100">
              ...
              <div className="p-3 bg-white flex items-center justify-between gap-3">
                <div className="space-y-0.5">
                  <span className="font-bold text-slate-900">
                    {t.pocReportCriterion2Title}
                  </span>
                  <p className="text-[11px] text-slate-500">
                    {t.pocReportCriterion2Desc}
                  </p>
                </div>
                <span className="px-2 py-1 rounded bg-emerald-100 text-emerald-800 font-bold text-[11px] flex-shrink-0">
                  {t.pocReportPassBadge}
        ...
        <div className="px-6 py-4 border-t border-slate-200 bg-slate-50 flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            >
              <span>{t.pocReportConvertBtn}</span>
